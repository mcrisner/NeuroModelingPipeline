function modeling_func_connectome(base_controls, pred_table, resp_table, sub_list, outputfile, vars_of_interest, adjustments, lme, mixed_effect_var, interaction, logistic)

% Description:
%   This function models the relationship between a set of predictors and various connectome 
...
%   resp_table        - Table containing response (connectome properties) data with subject IDs.
...
%   2. Defines the list of connectome properties to model.
%   3. Combines puberty stage 4 and 5 into a single group.
%   4. Constructs model data by combining predictor and response tables.
%   5. For each predictor and connectome property, fits a linear or mixed-effects model.
...
% Define list of properties to evaluate (transitivity modeled but omitted from results)
prop_names = {'median_connectivity', 'natural_connectivity', 'transitivity', ...
              'globalEfficiency', 'clusterCoef_global', 'modularity', 'svd_eigenvalue', ...
              'small_worldness', 'fragility', 'segregation'};

% Combine puberty stage 4 and 5 into one group (4)
pred_table.pds_score(pred_table.pds_score == 5) = 4;

% Extract predictor data from main table
...
% Extract relevant data from the response table and reformat subject IDs
resp_table.Properties.RowNames = strrep(resp_table.Properties.RowNames, 'sub-NDAR', 'NDAR_');
resp_table = resp_table(:, prop_names);
resp_table.src_subject_id = resp_table.Properties.RowNames;
...
for var_idx = 1:numel(vars_of_interest)
    ...
    for prop_idx = 1:numel(prop_names)
        ...
        disp(['Predictor: ' vars_of_interest{var_idx} '    Outcome: ' prop_names{prop_idx}]);
        warning('OK');

        % Combine the predictor data and props data for creating the complete model table
        model_data = innerjoin(pred_data, resp_table(:, {'src_subject_id', prop_names{prop_idx}}), 'Keys', 'src_subject_id');
        ...
        if strcmp(lastwarn, 'OK')
            ...
            adj_stat_tbl = array2table(NaN(length(mdl.CoefficientNames)-2, length(adj_stat_lst)), 'VariableNames', adj_stat_lst, 'RowNames', controls);
        ...
        all_stats.(vars_of_interest{var_idx}).(prop_names{prop_idx}) = adj_stat_tbl;

    end

    % Add table to the main output structure
    prop_stats.(vars_of_interest{var_idx}) = stat_tbl;
    ...
    for var = 1:width(p_values)
        ...
        if var == 1
            prop_stats.(vars_of_interest{var_idx}).qvalue = q_values;
        else
            for prop_idx = 1:numel(prop_names)
                all_stats.(vars_of_interest{var_idx}).(prop_names{prop_idx}).qvalue(var-1) = q_values(prop_idx);
            end
        end
    end
end
